      ...
      if (response.statusCode == 200) {
        for (int i = 0; i < groupsMap.length; i++) {
          ...
          for (var j = 0; j < groupsMap[i]["group_members"].length; j++) {
            User user = await _profileController.fetchThisUser(groupsMap[i]["group_members"][j]["member"]);
            members.add(user);
          }
        ...
        for (int i = 0; i < groupExpensesMap.length; i++) {
          ...
          for (var j = 0; j < groupExpensesMap[i]["settlement"].length; j++) {
            Settlement s = Settlement(
                ...
                amount: double.parse(groupExpensesMap[i]["settlement"][j]["amount"]) * 1.0
            );
            settlements.add(s);
          }
          for (var j = 0; j < groupExpensesMap[i]["paid_by"].length; j++) {
            PaidBy p = PaidBy(
                ...
                amount: double.parse(groupExpensesMap[i]["paid_by"][j]["amount"])* 1.0
            );
            paidBys.add(p);
          }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var j = 0; j < groupsMap["group_members"].length; j++) {
          User user = await _profileController.fetchThisUser(groupsMap["group_members"][j]["member"]);
          members.add(user);
        }
