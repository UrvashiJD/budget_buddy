...
class ProfileController extends GetxController {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < transactions.length; i++) {
          fetchedDates.add(DateTime(transactions[i].year, transactions[i].month,
              transactions[i].day));
        }
  ...
  Future addTransaction(Map<String, dynamic> data, String path) async {
    ...
    try {
      ...
      for (int i = 0; i < transactions.length; i++) {
        fetchedDates.add(DateTime(
            transactions[i].year, transactions[i].month, transactions[i].day));
      }
  ...
  Future deleteTransaction(String transactionID, String billSrc) async {
    ...
    try {
      ...
      balance.value = income.value - expense.value;
      transactions
          .removeWhere((element) => element.id == deletedTransaction.id);
    } catch (e) {
      rootScaffoldMessengerKey.currentState!.showSnackBar(SnackBar(
        content: Text(e.toString()),
      ));
  ...
  Future editImageTransaction(Map<String, dynamic> data, String transactionID,
      String billPath, String billImage) async {
    var url = Uri.parse(baseURL + '/transactions/$transactionID');
    ...
    try {
      if (billPath != "" && billImage == "") {
        ...
        await uploadMyTransactionBill(
            tID: transactionID, billPath: billPath);
  ...
  Future editTransaction(Map<String, dynamic> data, String transactionID,
      String billPath, String billImage) async {
    var url = Uri.parse(baseURL + '/transactions/$transactionID');
  ...
  void fetchMyContacts() async {
    ...
    if (await FlutterContacts.requestPermission()) {
      ...
      for (var contact in phoneContacts) {
        if (contact.phones.isNotEmpty) {
          allContacts
              .add(contact.phones.first.number.removeAllWhitespace.trim());
        }
      }
    }
    ...
    for (var fetchedUser in allFetchedUser) {
      if (allContacts
          .contains(fetchedUser.userMobile.removeAllWhitespace.trim())) {
