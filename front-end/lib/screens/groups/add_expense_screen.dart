...
class AddExpenseScreen extends StatefulWidget {
  ...
  const AddExpenseScreen({Key? key, required this.g,required this.callback}) : super(key: key);
                    ...
                    if (formKey.currentState!.validate()) {
                      ...
                      if (isPaidByDetailsAdded) {
                        ...
                        for (var e in paidControllers) {
                          paidAmount.add(double.parse(e.text.isEmpty
                                  ? "0"
                        ...
                        for (int i = 0; i < widget.g.members.length; i++) {
                          ...
                          if (i == widget.g.members.length - 1) {
                            paidBy = paidBy +
                                "${widget.g.members[i].userID}=${paidAmount[i]}";
                          } else {
                            paidBy = paidBy +
                                "${widget.g.members[i].userID}=${paidAmount[i]},";
                          }
                        ...
                        while (i < j) {
                          debt = min<double>(
                              -sortedValuesPaid[i], sortedValuesPaid[j]);
                          ...
                          if (i == j - 1) {
                            settlements = settlements +
                                "${sortedPeople[i]}:${sortedPeople[j]}=${debt.toPrecision(2)}";
                          } else {
                            settlements = settlements +
                                "${sortedPeople[i]}:${sortedPeople[j]}=${debt.toPrecision(2)},";
                          }
                        ...
                        showErrFlushBar(
                            context, "Warning", "Add valid paid by amount");
                      }
                    }
                  },
                        ...
                        ListTile(
                          ...
                          trailing: TextButton(
                            onPressed: () {
                              ...
                              double expAmount = double.parse(amountController
                                  .text
                                  .toLowerCase()
                                  .trim()
                                  .toString());
                  ...
                  children: [
                    ...
                    selectedValue == 0
                        ...
                        : Center(
                            child: Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: remainingSplitAmount < 0
                                  ? Text(
                                      "₹ $remainingSplitAmount exceeding",
                                      style: TextStyle(
                                        color: remainingSplitAmount == 0
                                            ? kSuccessColor
                                            : kErrorColor,
                                        fontWeight: FontWeight.bold,
                                      ),
