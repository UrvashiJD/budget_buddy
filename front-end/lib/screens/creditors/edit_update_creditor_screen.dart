...
import '../../controllers/creditors_controller.dart';
...
class UpdateDeleteCreditorScreen extends StatefulWidget {
  ...
  const UpdateDeleteCreditorScreen({Key? key, required this.creditor})
      : super(key: key);
...
class _UpdateDeleteCreditorScreenState
  ...
  final CreditorsController _creditorsController =
      Get.put(CreditorsController());
  ...
  void initState() {
    ...
    finalDate = DateTime(
        widget.creditor.year, widget.creditor.month, widget.creditor.day);
          ...
          if (formKey.currentState!.validate()) {
            if (_creditorMobileController.text.length == 10) {
              ...
              await _creditorsController.editCreditor(
                  updatedCreditor, widget.creditor.id);
              stopLoader();
                          ...
                          Expanded(
                            child: Align(
                              ...
                              child: IconButton(
                                onPressed: () async {
                                  showLoader(context);
                                  await _creditorsController
                                      .deleteCreditor(widget.creditor.id);
                                  stopLoader();
                                  Navigator.of(context).pop();
                                },
                                icon: const Icon(
